...
class PerturbationResults:
    ...
    def plot_trajectories(self,
        ...
        xu = np.array(self.similarities['weights'])
        mask = np.isfinite(xu)
        ...
        xc = np.array(self.similarities_clipped['weights'])
        mask = np.isfinite(xc)
        ...
        plt.xlabel(f'Perturbation Amount')
        ...
        fig = plt.figure(figsize=(10, 5))
        # Get and plot the X sets
        xu = np.array(self.similarities['activations'])
        mask = np.isfinite(xc)
        plt.scatter(np.array(range(len(xu)))[mask], xu[mask],
                    marker='o',
                    color='r',
        ...
        # clipped
        # clipped similarities
        xc = np.array(self.similarities_clipped['activations'])
        mask = np.isfinite(xc)
        plt.scatter(np.array(range(len(xu)))[mask], xc[mask],
                    marker='o',
                    color='b',
        ...
        # Get and plot the X sets
        xu = np.array(self.distances['weights'])
        mask = np.isfinite(xc)
        plt.scatter(np.array(range(len(xu)))[mask], xu[mask],
                    marker='o',
                    color='r',
        ...
        # clipped distances
        xc = np.array(self.distances_clipped['weights'])
        mask = np.isfinite(xc)
        plt.scatter(np.array(range(len(xu)))[mask], xc[mask],
                    marker='o',
                    color='b',
...
class Perturbation:
    ...
    def run_perturbation_training(self, layers, perturbation_level, w_clip_val=30, a_clip_val=64, plot=True):
        ...
        j_col = list(range(self.columns[j])) if self.columns else None
        j_row = list(range(self.rows[j])) if self.rows else None
        ...
        trial_loaders = subset_class_loader([], mod_ind=list(range(self.n_classes)),
                                            ...
                                            noise=self.noise, var=self.var, random_pixels=j_pix)
    ...
    def __call__(self, img_tensor):  # this should always come after the toTensor transform!
        ...
        if self.random_pix:
            ...
            for index in indices:
                ...
                img_tensor[:, index[0], index[1]] = add_val
